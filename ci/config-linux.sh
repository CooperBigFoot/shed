#!/usr/bin/env bash
# Builds the native dependency stack (GDAL + friends) for pyshed manylinux wheels.
#
# Mirrors the macOS stack in ci/config.sh, but targets Linux x86_64 inside
# cibuildwheel's manylinux2014 container.
...
BUILD_PREFIX="${BUILD_PREFIX:-/project/pyshed_libs}"
OS="$(uname -s)"
ARCH="$(uname -m)"
lib_ext="so"
...
if [[ "$OS" != "Linux" ]]; then
    echo "Unsupported OS: $OS — this script targets Linux only." >&2
    exit 1
fi
...
function build_curl {
    ...
    (cd "${CURL_FNAME}" &&
        LD_LIBRARY_PATH="${LD_LIBRARY_PATH:-}:$BUILD_PREFIX/lib" ./configure $flags &&
        make -j4 &&
        make install)
    ensure_rpath "libcurl*.so*"
    touch curl-stamp
}
